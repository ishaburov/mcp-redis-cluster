...
def parse_redis_cluster_startup_nodes(
    raw: str, default_port: int
) -> list[dict[str, object]] | None:
    """Parse REDIS_CLUSTER_NODES (comma-separated host:port) for RedisCluster."""
    text = (raw or "").strip()
    if not text:
        return None
    ...
    for segment in text.split(","):
        ...
        if ":" in part:
            ...
            nodes.append({"host": part, "port": int(default_port)})
    return nodes or None


REDIS_CFG["cluster_startup_nodes"] = parse_redis_cluster_startup_nodes(
    os.getenv("REDIS_CLUSTER_NODES", ""),
    REDIS_CFG["port"],
)
